<!DOCTYPE html>
<html>
	<head>
		<title>MIT Guest House</title>
		<link rel="icon"  href="mit.jpeg">
		<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
		<style>
			*
			{
				color:saddlebrown;
				font-family: Georgia, serif;
			}
			a:link,a:visited,a:hover,a:active
			{
				text-decoration: none;
				color:saddlebrown;
			}
		</style>	
	</head>
	<body class="bg-light">
		<div class="p-3" style="background-color:linen; display: flex; flex-direction: row; justify-content: space-between;">
			<div style="display:flex; flex-direction:row;">
				<img src="logo.png" height="50px" width="50px" class="m-3">
				<p class="p-1" style="font-size:38px;"> MIT Guest House </p>
			</div>
			<div class="pt-4" style="font-size:20px;">
				<a href="index.php">Home</a>&emsp;
				<a href="aboutus.php">About Us</a>&emsp;
				<a href="ourgallery.php">Our Gallery</a>&emsp;
				<a href="reservation.php">Make a Reservation</a>&emsp;
				<a href="contactus.php">Contact Us</a>
			</div>
		</div>

		<div class="container py-5">
			<h1 class="mb-5">Our Gallery</h1>
			<h2>Destination</h2><hr style="height:5px;">
			<div class="display:flex; flex-direction:column">
				<div class="display:flex; flex-direction:row;">
					<img class="m-5" height="250px" width="350px" src="carousel/frontgate.jpg">
					<img class="m-5" height="250px" width="350px" src="carousel/hostelgate.jpg">
				</div>
			</div>
			<h2>Accomodation</h2><hr style="height:5px;">
			<div class="display:flex; flex-direction:column">
				<div class="display:flex; flex-direction:row;">
					<img class="m-5" height="250px" width="350px" src="carousel/room.png">
					<img class="m-5" height="250px" width="350px" src="carousel/room1.jpg">
				</div>
				<div class="display:flex; flex-direction:row;">
					<img class="m-5" height="250px" width="350px" src="carousel/room2.jpg">
					<img class="m-5" height="250px" width="350px" src="carousel/corridor.jpg">
				</div>
			</div>
			<h2>Dining</h2><hr style="height:5px;">
			<div class="display:flex; flex-direction:column">
				<div class="display:flex; flex-direction:row;">
					<img class="m-5" height="250px" width="350px" src="carousel/dining.jpg">
				</div>
			</div>
		</div>
		
		<div class="p-3 m-0" style = "background-color:linen; text-align:right;">
			<label>&copy; Copyright 2023 </label>
		</div>
	</body>	
</html>