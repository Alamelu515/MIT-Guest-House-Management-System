<!DOCTYPE html>
<!--To show people who have reserved rooms or the pendings-->
<html>
	<head>
	...
	<body class="bg-light" style="font-size:20px;">
		...
		<div class="container py-5">
			<h1 class="mb-5">Reservations - Pendings</h1>
			<?php
				require_once 'connect.php';
				$q_p = $conn->query("SELECT COUNT(*) as total FROM `transaction` WHERE `status` = 'Pending'") or die($conn->error);
				$f_p = $q_p->fetch_array();
				$q_ci = $conn->query("SELECT COUNT(*) as total FROM `transaction` WHERE `status` = 'Check In'") or die($conn->error);
			...
			<table id = "table" class = "table table-bordered">
				...
				<tbody>
					...
					<tr>
						...
						<td><center>
							<a class = "btn btn-dark text-light" href = "confirm_reserve.php?transaction_id=<?php echo $fetch['transaction_id']?>">Check In</a> 
							<a class = "btn btn-dark text-light" href = "delete_pending.php?transaction_id=<?php echo $fetch['transaction_id']?>">Discard</a>
						</center></td>
					</tr>
					<?php
						}
